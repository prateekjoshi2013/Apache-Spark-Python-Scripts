...
def simpleTokenize(string):
    ...
    """
    string_PuncRem= "".join(s for s in string if s not in('!','.',':','/') )
    string_Tokenized = (re.split(split_regex,(string).lower()))
    
    return [x for x in string_Tokenized if x!='' and x!=' ']
...
def tokenize(string):
    ...
    """
    tokenized_String=simpleTokenize(string)
    stopWords_Removed=[s for s in tokenized_String  if s not in (stopwords) ]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda (key,value):(key,tokenize(value)))
googleRecToToken = googleSmall.map(lambda (key,value):(key,tokenize(value)))
...
def countTokens(vendorRDD):
    ...
    """
    #vendorRDD.map(lambda (key,value):int(value))
    #count=vendorRDD.reduce(lambda a,b:a+b)
    vendorRDD2=vendorRDD.map(lambda (key,value):(key,len(value)))
    count=vendorRDD2.map(lambda (key,value):value).collect()
    return sum(count)
    

#print countTokens(amazonRecToToken)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    rec_List=vendorRDD.sortBy(lambda s: -1*len(s[1])).collect()
    print  rec_List[0][0]
    return rec_List
...
def tf(tokens):
    ...
    for x in tokens_set:
        tf_dict.update({x:float(tokens.count(x))/len(tokens)})
    return tf_dict
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (key,value):list(set(value)))
    tokenCountPairTuple = uniqueTokens.map(lambda key: (key,1))
    
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a,b:a+b).map(lambda (key,value):(key,float(N)/value))
    #print tokenSumPairTuple
...
def tfidf(tokens, idfs):
    ...
    for t in tokens:
        tfIdfDict.update({t:(tfs[t]*idfs[t])})
    return tfIdfDict
...
def norm(a):
    ...
    sqrt={}
    for t in a:
        sqrt.update({t:a[t]*a[t]})
    return math.sqrt(sum(sqrt.values()))
...
def cossim(a, b):
    ...
    """
    inter_sum=dotprod(a,b)/(norm(a)*norm(b))
    return inter_sum 
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (url,ids,sim): (ids+' '+url,sim))
gldStndrd=goldStandard.map(lambda (key,value):key).collect()
trueDupsRDD = sims.join(goldStandard)
#print trueDupsRDD.count()
trueDupsCount = trueDupsRDD.count()
avgSimDups = sum([x[1][0] for x in trueDupsRDD.collect()])/146
print avgSimDups
nonDupsRDD = (sims
             .filter(lambda (key,value):key not in gldStndrd).collect())
avgSimNon = sum([x[1] for x in nonDupsRDD])/len(nonDupsRDD)
print avgSimNon
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (key,value):(key,tokenize(value)))
googleFullRecToToken = google.map(lambda (key,value):(key,tokenize(value)))
...
idfsFullWeights = dict(idfs(fullCorpusRDD).collect())
idfsFullBroadcast = idfsFullWeights
#print idfsFullWeights
# Pre-compute TF-IDF weights.  Build mappings from record ID weight vector.
amazonWeightsRDD = amazonFullRecToToken.map(lambda (key,value):(key,tfidf(value,idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda (key,value):(key,tfidf(value,idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (key,value):(key,norm(value))).collect()
amazonNormsBroadcast =sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda (key,value):(key,norm(value))).collect()
...
def invert(record):
    ...
    lt=[(x,record[0]) for x in record[1].keys()]
...
commonTokens = (amazonInvPairsRDD
                ...
                .cache())
#print amazonInvPairsRDD.collect()
...
def fastCosineSimilarity(record):
    ...
    #amazonWeightsBroadcast.value[record[0][0]]
    googleRec = record[0][1]
    #googleWeightsBroadcast[record[0][0]]
    tokens = record[1]
    #s = dotprod(amazonWeightsBroadcast.value[amazonRec],googleWeightsBroadcast[googleRec])
    value = cossim(amazonWeightsBroadcast.value[amazonRec],googleWeightsBroadcast.value[googleRec])
