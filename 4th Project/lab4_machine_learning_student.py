...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    NumberOfRatings=len(IDandRatingsTuple[1])
    averageRating=float(sum(IDandRatingsTuple[1]))/NumberOfRatings
    return (IDandRatingsTuple[0],(NumberOfRatings,averageRating))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (movieId,(movieTitle,(rCount,movieAvg))):(movieAvg,movieTitle,rCount)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avgRating,movieName,rCount):rCount>500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda ((UserID, MovieID), (Rating1,Rating2)): ((UserID, MovieID), (Rating1-Rating2)**2)))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.reduceByKey(lambda a,b:a+b).map(lambda (key,value):value).sum()
...
# TODO: Replace <FILL IN> with appropriate code
myUserID = 0
#print movieIDsWithAvgRatingsRDD.take(50)
# Note that the movie IDs are the *last* number on each line. A common error was to use the number of ratings as the movie ID.
myRatedMovies = [(myUserID,2,5),(myUserID,3,4.5),(myUserID,4,4),(myUserID,5,3),(myUserID,6,1),(myUserID,7,2),(myUserID,8,4),(myUserID,9,2),(myUserID,10,5),(myUserID,11,4)
...
# TODO: Replace <FILL IN> with appropriate code
trainingWithMyRatingsRDD = myRatingsRDD +trainingRDD
...
myUnratedMoviesRDD = (
                      ...
                      .filter(lambda (x,y):y not in [x[1] for x in myRatedMovies] ).take(1))
