...
dailyHosts = (dayGroupedHosts.map(lambda (key,value): (key,len(list(set(value)))))).cache()
#print dailyHosts
dailyHostsList = dailyHosts.takeOrdered(30,lambda (key,value):key)
...
daysWithHosts = sorted(dailyHosts.map(lambda (key,value):key).collect())
...
sortedByDay =(groupedByDay.map(lambda (key,value): (key,len(value)/len(list(set(value))))))
avgDailyReqPerHost =(sortedByDay).cache()
#sorted ([(x[0],(x[1]/sum([y[1] for y in sortedByDay]))) for x in sortedByDay])
#print [y[1] for y in sortedByDay]

avgDailyReqPerHostList = sorted(avgDailyReqPerHost.take(30))
...
daysWithAvg = [x[0] for x in sorted(avgDailyReqPerHost.collect())]
avgs = [x[1] for x in sorted(avgDailyReqPerHost.collect())]
...
#(([x[0] for x in badUniqueEndpoints ]))
...
errDateSorted = (errDateSum).cache()

errByDate = sorted((errDateSorted).collect())
...
daysWithErrors404 = [x[0] for x in sorted(errDateSorted.collect())]
errors404ByDay = [x[1]for x in sorted(errDateSorted.collect())]
...
topErrDate = errDateSorted.takeOrdered(5,lambda (key,value):-value)
...
hourCountPairTuple = badRecords.map(lambda log:(log.date_time.hour,1)).sortByKey()

#print hourCountPairTuple
hourRecordsSum = hourCountPairTuple.reduceByKey(lambda a,b: a + b )

hourRecordsSorted = sc.parallelize(hourRecordsSum.collect()).cache()

errHourList =sorted(hourRecordsSorted.collect())
...
hoursWithErrors404 = [x[0] for x in sorted(hourRecordsSorted.collect())]
errors404ByHours = [x[1] for x in sorted(hourRecordsSorted.collect())]
